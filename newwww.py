...
for Nv in Nv_list:
    ...
    Y_train = u_train[:, :Nv] + noise_level * np.random.randn(*u_train[:, :Nv].shape)
    Y_test = u_test[:, :Nv] + noise_level * np.random.randn(*u_test[:, :Nv].shape)
    ...
    dataset = TensorDataset(Y_train_tensor,X_train_tensor)
    train_loader = DataLoader(dataset, batch_size=128, shuffle=True)

    G = Generator(y_dim=y_dim, z_dim=z_dim, x_dim=x_dim).to(device)
    D = Discriminator(y_dim=y_dim, x_dim=x_dim).to(device)
    ...
    dataset = TensorDataset(Y_test_tensor, X_test_tensor)
